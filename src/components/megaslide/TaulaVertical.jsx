...
export function TaulaVerticalP1({ grid = null, stripe = null, fletxes = null, selector = null }) {
  ...
  return (
    ...
    >
      ...
      <div data-taula-cela="7-9+12-14" style={{ ...CELA, gridColumn: '2 / 5', gridRow: '2 / 4', justifyContent: 'flex-start', alignItems: 'flex-start', marginLeft: '-20px', marginRight: '-20px' }}>{stripe || 'Stripe'}</div>
...
export function TaulaVerticalP2({ graella = null, colleccions = null, colors = null, selector = null, stripe = null }) {
  // La taula de la pagina 2 va SENSE contorns visibles: la vora es transparent
  // (no `none`) perque la geometria de les caselles no es mogui.
  const CELA2 = { ...CELA, border: '1px solid transparent' };
  return (
    <div
    ...
    >
      <div data-taula-cela="1-5" style={{ ...CELA2, gridColumn: '1 / -1', gridRow: '1' }}>{graella || 'Graella dibuixos 16x4'}</div>
      {/* Les caselles veïnes s'ajusten a la correguda de 10 px: la de
          ...
          tornin a tocar-se. */}
      <div data-taula-cela="6-11" style={{ ...CELA2, gridColumn: '1', gridRow: '2 / 4', marginRight: '10px' }}>
      ...
      <div data-taula-cela="7" style={{ ...CELA2, gridColumn: '2', gridRow: '2', transform: 'translateX(-10px)', marginRight: '20px', alignItems: 'flex-end' }}>{selector || 'Selector b/c/n'}</div>
      ...
      <div data-taula-cela="8-10+13-15" style={{ ...CELA2, gridColumn: '3 / 6', gridRow: '2 / 4', marginLeft: '-30px', justifyContent: 'flex-start', alignItems: 'flex-start' }}>{stripe || 'Stripe'}</div>
      <div data-taula-cela="12" style={{ ...CELA2, gridColumn: '2', gridRow: '3', transform: 'translateX(-10px)', marginRight: '20px', alignItems: 'flex-end' }}>{colors || 'Graella colors 4x4'}</div>
    </div>
  );
}
